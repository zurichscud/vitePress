...
| 值          | 含义                 | 触发                                                |
| ----------- | -------------------- | --------------------------------------------------- |
| `'active'`  | actor 正在运行       | 进入state                                           |
| `'done'`    | actor 已正常完成     | Promise resolve，或者到达标识为`type:'final'`的状态 |
| `'error'`   | actor 发生错误并终止 | Promise reject                                      |
| `'stopped'` | actor 被手动 stop    | 离开 state                                          |

到达done时，**这个 actor 的工作已经结束，生命周期终结**。它 **不能再接收事件，也不能再发生状态变化**。
