...
被生成的 Actor。手动启动，手动停止，生命周期更自由

### 核心特征

- 在 **transition / action 中 spawn**
- 不依赖某个具体状态
- 需要手动 `stop`
- 适合：**数量不固定、长期存在的实体**

| 对比点   | Invoked Actor | Spawned Actor |
| -------- | ------------- | ------------- |
| 生命周期 | 跟随状态      | 手动控制      |
| 创建方式 | `invoke`      | `spawn`       |
| 数量     | 固定、有限    | 动态、不确定  |
...
在 actor 模型中，actor 是可以相互通信的对象。它们是独立的"活跃"实体，通过异步消息传递进行通信。在 XState 中，这些消息被称为 *[events](https://stately.ai/docs/transitions)*。

- 一个Actor拥有自己内部的、封装的状态，该状态只能由参与者自身更新。参与者可以选择在接收到消息时更新其内部状态，但不能被任何其他实体更新。
- Actor通过异步发送和接收事件与其他参与者进行通信。
- Actor 一次只处理一条消息。它们有一个内部"邮箱"，就像事件队列一样，按顺序处理事件。
- 内部参与者状态不会在参与者之间共享。参与者共享其内部状态的任何部分的唯一方法是：
  - 向其他参与者发送事件
...
当 actor 接收到事件时，其内部状态可能会发生变化。当状态转换发生时，actor 可能会发出一个 **快照** 。您可以通过 `actor.getSnapshot()` 同步读取 actor 的快照，或者通过 `actor.subscribe(observer)` 订阅快照。
