...
| 值           | 对齐方向       | 默认height | 效果说明                                               |
| ------------ | -------------- | ---------- | ------------------------------------------------------ |
| `stretch`    | 拉伸（默认值） | 容器高度   | **若子元素未设置高度**（主轴:row），会被拉伸以填满容器 |
| `flex-start` | 交叉轴起点对齐 | 内容高度   | 子项都靠交叉轴的起点（如上方）对齐                     |
| `flex-end`   | 交叉轴终点对齐 | 内容高度   | 子项都靠交叉轴的终点（如下方）对齐                     |
| `center`     | 居中对齐       | 内容高度   | 子项在交叉轴中间居中                                   |
| `baseline`   | 基线对齐       | 内容高度   | 子项的文本基线（文字底部）对齐                         |
