...
在 **Spring Boot** 中，**Starter（场景启动器）** 是一种用于简化依赖管理的机制。它的核心作用是让你在项目中快速引入一整套常用依赖，而无需手动查找和添加每个单独的库。可以理解为“依赖的依赖包”，一站式帮你把相关技术栈准备好。
...
第三方的 starter 不应该以 `spring-boot` 开头，因为这个前缀保留给官方 Spring Boot 工件。第三方 starter 通常以项目名称开头。例如，一个名为 `thirdpartyproject` 的第三方 starter 项目，通常会命名为 `thirdpartyproject-spring-boot-starter`。